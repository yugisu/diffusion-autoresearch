...
from backbone import DEFAULT_SSL_CKPT, load_base_backbone, load_ssl_backbone
...
SSL_CKPT_DEFAULT = DEFAULT_SSL_CKPT
...
class Config:
    ...
    backbone_init: str = "ssl"  # one of: "ssl", "base"
...
class DinoCrossViewRetrieverST2(pl.LightningModule):
    ...
    def __init__(self, cfg: Config):
        ...
        if cfg.backbone_init == "ssl":
            self.backbone = load_ssl_backbone(cfg.ssl_ckpt, cfg.model_name)
        elif cfg.backbone_init == "base":
            self.backbone = load_base_backbone(cfg.model_name)
        else:
            raise ValueError(f"Unsupported backbone_init='{cfg.backbone_init}'. Expected 'ssl' or 'base'.")
...
def parse_args() -> Config:
    ...
    parser.add_argument("--backbone-init", type=str, choices=["ssl", "base"], default=Config.backbone_init)
...
def main():
    ...
    print(f"Backbone init:   {cfg.backbone_init}")
    if cfg.backbone_init == "ssl":
