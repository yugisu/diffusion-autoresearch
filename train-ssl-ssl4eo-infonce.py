"""
Controlled ablation: SSL4EO-S12 training data with InfoNCE objective.

Differences from train-ssl-visloc.py (baseline):
  - Dataset: SSL4EO-S12 S2RGB seasonal patches (China region) instead of VisLoc sat chunks
  - Positives: two temporal timestamps of the same location instead of same-image cross-scale crops

All other aspects kept identical to train-ssl-visloc.py:
  - Objective: symmetric InfoNCE (temperature=0.07)
  - No projection head (proj_dim=0)
  - Augmentation: strong anchor aug (ColorJitter b=0.5 c=0.5 s=0.3 h=0.1, grayscale, blur),
                  mild positive aug (ColorJitter b=0.3 c=0.3) — exact same policy
  - LoRA rank=16, last 4 blocks only
  - AdamW + cosine LR with 2-epoch linear warmup
  - Validation: VisLoc flight 03, R@1 with 4-rotation TTA

Structural differences forced by WebDataset:
  - steps_per_epoch=200: defines epoch size (SSL4EO has 244K+ samples, natural epochs are too long)
  - val_dataloader absent: PL IterableDataset interaction prevents epoch-end validation; eval is
    done manually in on_train_epoch_end (same pattern as train-ssl.py)

Usage:
  uv run train-ssl-ssl4eo-infonce.py > run.log 2>&1
...
class Config:
    ...
    warmup_epochs: int = 2
    proj_dim: int = 0  # disabled — InfoNCE on raw CLS embeddings

    georank_weight: float = 0.0
    cosine_t0: int = 0
    ...
    max_epochs: int = 13
    max_steps: int = -1
    steps_per_epoch: int = 200  # defines epoch size for WebDataset; 0 = natural exhaustion
    time_budget_hours: float = 0.0  # 0 = no limit
    ...
    ssl4eo_max_cloud_cover: float = 0.5
    ssl4eo_min_brightness: float = 30.0

    wandb_project: str = "autoresearch-ssl-dinov3-ablation"
    wandb_run_name: str | None = "ssl4eo-temporal-infonce"


# -----------------------------------------------------------------------------
# LoRA — identical to train-ssl-visloc.py
# -----------------------------------------------------------------------------
...
class TimeBudgetCallback(pl.Callback):
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        ...
        if elapsed >= self.budget_seconds:
            print(f"[TimeBudget] {elapsed / 3600:.2f}h elapsed ≥ {self.budget_seconds / 3600:.1f}h budget. Stopping.")
            trainer.should_stop = True


# -----------------------------------------------------------------------------
# SSL4EO-S12 data pipeline — single temporal pair per location
# -----------------------------------------------------------------------------
...
def build_ssl4eo_ssl_pipeline(
    ...
    """
    WebDataset pipeline yielding (anchor, positive, coords, coords) batches.

    anchor  — season t1, zoomed-in crop (25-50%); anchor_transform applied
    positive — season t2 (different season), full-scale crop (75-100%); positive_transform applied

    The pipeline self-batches; DataLoader must be created with batch_size=None.
    """
    root = Path(data_root)
    ...
    def to_ssl_pair(sample):
        ...
        T = bands.shape[0]
        if T >= 2:
            t_anchor, t_pos = np.random.choice(T, 2, replace=False).tolist()
        else:
            t_anchor, t_pos = 0, 0

        img_a = Image.fromarray(np.transpose(bands[t_anchor], (1, 2, 0)))
        img_p = Image.fromarray(np.transpose(bands[t_pos], (1, 2, 0)))
...
class VisLocSSLDataModule(pl.LightningDataModule):
    def __init__(self, cfg: Config):
        ...
        # Identical to train-ssl-visloc.py
        anchor_aug = [
    ...
    def train_dataloader(self):
        return DataLoader(
            ...
            batch_size=None,  # WebDataset self-batches
            shuffle=False,
    ...
    # val_dataloader intentionally absent: PL IterableDataset interaction
    # prevents epoch-end validation from firing. Eval is done manually in
    # DinoSSLRetriever.on_train_epoch_end via _eval_retrieval().


# -----------------------------------------------------------------------------
# Projection head (kept for completeness; disabled in this ablation)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Lightning model — InfoNCE training + manual retrieval eval
# -----------------------------------------------------------------------------
...
class DinoSSLRetriever(pl.LightningModule):
    def __init__(self, cfg: Config):
        ...
        self.proj_head = ProjectionHead(cfg.embedding_dim, cfg.proj_dim) if cfg.proj_dim > 0 else None
    ...
    def _infonce_loss(self, q: torch.Tensor, k: torch.Tensor) -> torch.Tensor:
        """Symmetric InfoNCE — identical to train-ssl-visloc.py."""
    ...
    def _georank_loss(self, embs: torch.Tensor, coords: torch.Tensor) -> torch.Tensor:
        """Flat-earth GeoRank regularization — identical to train-ssl-visloc.py."""
        dlat = coords[:, 0:1] - coords[:, 0:1].T
        dlon = coords[:, 1:2] - coords[:, 1:2].T
        geo_dist = torch.sqrt(dlat**2 + dlon**2 + 1e-10)
        sim = embs @ embs.T
    ...
    def training_step(self, batch, batch_idx):
        anchor, positive, anchor_coords, _ = batch
    ...
    def _eval_retrieval(self) -> dict:
        """Manual retrieval eval — bypasses PL's validation loop."""
    ...
    def configure_optimizers(self):
        ...
        if t0 > 0:
            ...
            def lr_lambda(epoch):
                ...
                pos = (epoch - warmup) % t0
                return 0.01 + 0.99 * 0.5 * (1 + math.cos(math.pi * pos / t0))
...
def parse_args() -> Config:
    parser = argparse.ArgumentParser(description="SSL4EO-S12 InfoNCE ablation (identical setup to train-ssl-visloc.py)")
    ...
    parser.add_argument("--ssl4eo-lat-min", type=float, default=Config.ssl4eo_lat_min)
    parser.add_argument("--ssl4eo-lat-max", type=float, default=Config.ssl4eo_lat_max)
    parser.add_argument("--ssl4eo-lon-min", type=float, default=Config.ssl4eo_lon_min)
    parser.add_argument("--ssl4eo-lon-max", type=float, default=Config.ssl4eo_lon_max)
...
def main():
    ...
    print("=" * 80)
    print("Ablation: SSL4EO-S12 temporal pairs + InfoNCE (same setup as train-ssl-visloc.py)")
    print(f"Model: {cfg.model_name}")
    print(f"SSL4EO root: {cfg.ssl4eo_root}")
    ...
    print(f"SSL4EO shards: {n_shards}  |  geo filter: {geo_filter}")
    print(f"Val flight: {VAL_FLIGHT}")
    print(f"LoRA rank={cfg.lora_rank}, alpha={cfg.lora_alpha}, last_n_blocks={cfg.lora_last_n_blocks}")
    print(f"VisLoc root: {cfg.visloc_root}")
    print(
        f"Train config: batch_size={cfg.batch_size}, steps_per_epoch={cfg.steps_per_epoch},"
        f" max_epochs={cfg.max_epochs}, num_workers={cfg.num_workers}"
    )
    print("=" * 80)
    ...
    ckpt_cb = ModelCheckpoint(
        dirpath="checkpoints/ssl-dinov3-ssl4eo-infonce",
        monitor="val/R@1",
        mode="max",
        save_top_k=1,
